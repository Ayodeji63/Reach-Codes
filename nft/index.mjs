...
const [accAlice, accBob, accClaire] = await stdlib.newTestAccounts(
  3,
  startingBalance
);
...
const makeOwner = (acc, who) => {
  ...
  return ctc.p.Owner({
    ...
    getAuctionProps: () => {
      console.log(
        `${who} starts the bidding at ${fmt(auctionProps[who].startingBid)}`
      );
    ...
    getBid: (price) => {
      if (price < bids[who].maxBid) {
        ...
        console.log(
          `${who} tries to bid ${fmt(bid)} (based on price: ${fmt(price)})`
        );
        return ["Some", bid];
      } else {
        return ["None", null];
