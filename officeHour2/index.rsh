...
export const main = Reach.App(() => {
    ...
    const Guests = new Map(Address, Bool);
    const [done, howMany] = parallelReduce([ false, 0 ])
    .invariant(Guests.size() == howMany, "how many is not accurate")
    .invariant(balance() == howMany * reservation, 'balance is not accurate')
      .while(!(done && howMany == 0))
      .api_(Guest.register, () => {
        check(!done, 'event started');
        check(isNone(Guests[this]), 'already register')
